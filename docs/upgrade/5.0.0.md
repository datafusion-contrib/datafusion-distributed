...
## 2. `TaskCountAnnotation::Desired` now stores `f64`

`TaskCountAnnotation::Desired` now stores `f64`. Fractional hints remain fractional while the
planner combines them and are rounded up when the final stage task count is resolved.
`DesiredTaskCountEventResponse::desired` accepts integer and floating-point values, so existing
integer call sites do not need casts. `maximum` remains an integer hard cap.

```rust
// Whole-number hints continue to compile unchanged.
Some(Ok(DesiredTaskCountEventResponse::desired(scan.shards.len())))

// Fractional hints can now be preserved until the planner needs a concrete count.
Some(Ok(DesiredTaskCountEventResponse::desired(0.25)))
```
