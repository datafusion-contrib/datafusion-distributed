...
For a resolver using the built-in gRPC transport, create its client with
`grpc::create_worker_client` and box it as the `WorkerChannel` implementation:
...
async fn get_worker_client_for_url(
    ...
    let channel = self.grpc_resolver.get_channel(url).await?;
    Ok(Box::new(grpc::create_worker_client(channel)))
}
```
