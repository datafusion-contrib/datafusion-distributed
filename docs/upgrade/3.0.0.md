...
For a resolver using the built-in gRPC transport, return the
`Box<dyn WorkerChannel>` produced by `grpc::create_worker_client` directly:
...
async fn get_worker_client_for_url(
    ...
    Ok(grpc::create_worker_client(channel))
}
```
