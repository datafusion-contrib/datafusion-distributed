...
## 4. Update DataFusion extension implementations

The DataFusion 55 dependency now points to commit
`46bd88a12538b8115209ad29252ee7aa94985966`, uses Arrow 59, and requires Rust
1.94 or newer.

Custom `PhysicalExtensionCodec` implementations must accept the new
`&dyn PhysicalProtoConverterExtension` argument in both `try_encode` and
`try_decode`. Use that converter when encoding or decoding nested DataFusion
plans, expressions, partitioning, or file-scan values. See
[Distribute a custom execution plan](../source/user-guide/04-distribute-custom-plan.md)
for the complete signatures.

Custom `ExecutionPlan` implementations must also implement
`apply_expressions`. A node with no expressions can return
`TreeNodeRecursion::Continue`; a node that owns expressions must invoke the
callback for those expressions using DataFusion's expression-tree helpers.

If a custom plan reads child statistics, migrate from `partition_statistics`
to `statistics_from_inputs` and declare the required child values through
`child_stats_requests`. Call `StatisticsContext::compute` when initiating a
statistics walk.
