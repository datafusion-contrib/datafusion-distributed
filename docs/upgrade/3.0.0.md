...
`DesiredTaskCountEventResponse::desired` now accepts an `f64`. Cast integer variables as shown
above and write integer-valued literals with a decimal point, such as `desired(3.0)`. Fractional
hints remain fractional while the planner combines them and are rounded up only when a concrete
stage task count is required.
