...
- **Registering a user codec now replaces an existing codec of the same concrete
  type.** If an application registered multiple configured instances of one
  codec type, combine their dispatch into one codec or wrap them in distinct
  concrete types. Coordinators and workers must still register distinct codec
  types in the same order because the wire format identifies them by position.
