...
messages added to the coordinator protocol. Encode/decode the new
`WorkerToCoordinatorMsg::TaskCompletedDynamicFilters` variant. The built-in gRPC
transport handles this automatically. `TaskDynamicFilter::expression` contains a typed
`datafusion_proto::protobuf::PhysicalExprNode`; custom transports should encode it only at their
wire boundary.

Runtime distributed dynamic filtering adds the
`WorkerToCoordinatorMsg::ProducedDynamicFilter` variant. Custom transports must forward this
message, preserving its typed `PhysicalExprNode` internally and serializing it only at the wire
boundary. They must also propagate `SetPlanRequest::dynamic_filter_report_ids`, which restricts
producer reports to filters with remote consumers. Exhaustive matches over
`WorkerToCoordinatorMsg` must add the new variant.

The coordinator-to-worker side also adds
`CoordinatorToWorkerMsg::ApplyDynamicFilter`. Custom transports must forward its typed merged
predicate to the selected task, serializing it only at the wire boundary. Exhaustive matches over
`CoordinatorToWorkerMsg` must add the new variant.
