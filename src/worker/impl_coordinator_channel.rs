use crate::common::{TreeNodeExt, completed_dynamic_filters};
...
use datafusion_proto::physical_plan::{AsExecutionPlan, DeduplicatingProtoConverter};
...
fn build_task_dynamic_filters(
    ...
    let filters: Vec<TaskDynamicFilter> = completed_dynamic_filters(plan, allowed_ids)?
        .into_iter()
        .map(|(expression_id, expression)| {
            Ok(TaskDynamicFilter {
                expression_id,
                expression: serialize_physical_expr(&expression, &codec)?.encode_to_vec(),
            })
        })
        .collect::<Result<_>>()?;
