...
mod tests {
    ...
    #[test]
    fn test_roundtrip_single_broadcast() -> datafusion::common::Result<()> {
        let codec = DistributedCodec;
        let ctx = create_context();

        let schema = schema_i32("h");
        let part = Partitioning::Hash(vec![Arc::new(Column::new("h", 0))], 4);
        let plan: Arc<dyn ExecutionPlan> =
            Arc::new(new_network_broadcast_exec(part, schema, dummy_stage()));
