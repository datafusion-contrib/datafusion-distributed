...
/// Discover and load TPC-DS queries, optionally filtered by query names
pub fn queries(query_filter: Option<&str>) -> Result<Vec<(String, String)>> {
    ...
    let filter_names: Vec<String> = query_filter
    ...
    for entry in entries {
        let entry = entry.map_err(|e| {
            internal_datafusion_err!("Failed to read directory entry: {}", e)
        })?;

        let path = entry.path();
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("sql") {
            if let Some(name) = path.file_stem().and_then(|s| s.to_str()) {
                if !filter_names.is_empty() && !filter_names.contains(&name.to_lowercase()) {
                    continue;
                }
