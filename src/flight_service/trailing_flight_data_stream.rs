    ...
    F: FnOnce() -> Result<Option<T>, FlightError>,
{
    // TODO: remove
    #[allow(dead_code)]
    pub fn new(on_complete: F, inner: S, debug: Option<usize>) -> Self {
        Self {
            inner,
    ...
    F: FnOnce() -> Result<Option<T>, FlightError>,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let debug = this.debug.clone();
        let mut partition = 0;
        if this.debug.clone().is_some() {
            partition = debug.unwrap();
        }
...
mod tests {
    ...
    async fn test_basic_streaming_functionality() {
        ...
        let trailing_stream = TrailingFlightDataStream::new(on_complete, inner_stream, None);
