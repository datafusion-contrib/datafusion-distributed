...
/// List of adjectives to use as a sourcej
const ADJECTIVE_LIST: &[&str] = &[
    "abandoned",
    "able",
    "absolute",
    "adorable",
...
struct Friendly {
    animals: Mutex<Vec<&'static str>>,
    adjectives: Mutex<Vec<&'static str>>,
}

impl Friendly {
    /// Create a new friendly name generator
    pub fn new() -> Self {
        Self {
            animals: Mutex::new(Vec::from(ANIMALS)),
            adjectives: Mutex::new(Vec::from(ADJECTIVE_LIST)),
        }
    }
    ...
    pub fn new_name(&self) -> Result<String> {
        ...
        let animals = self
            .animals
            .lock()
            .map_err(|e| anyhow!("Failed to lock animals: {}", e))?;
        let index = rng.gen_range(0..animals.len());
        let name = animals[index];

        // Get an adjective (removing it from the set)
