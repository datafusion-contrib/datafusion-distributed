...
fn _apply_network_boundaries(
        ...
        && node.partition_mode() == &PartitionMode::CollectLeft
    {
        ...
        // Force a stage break on probe side if it contains a shuffle to avoid partition mismatch.
        let final_probe_side = if contains_shuffle_boundary(&probe_side) {
            Arc::new(NetworkCoalesceExec::new(probe_side, 1)) as Arc<dyn ExecutionPlan>
        } else {
            probe_side
        };
        ...
        ctx.plan = ctx
            .plan
            .with_new_children(vec![build_side_broadcast, final_probe_side])?;
        return Ok(ctx);
    }
...
fn _distribute_plan_inner(
    ...
    let mut distributed = plan.clone().transform_down(|plan| {
        ...
        if let Some(node) = plan.as_any().downcast_ref::<PartitionIsolatorExec>() {
            ...
            // Continue traversal to convert any network boundaries in the subtree to Ready state.
        ...
        let stage = loop {
            ...
            // Skip network boundary when both stages have 1 task (can communicate in memory).
            // Exception: always keep NetworkBroadcastExec to enable caching for broadcast joins.
            let is_broadcast = plan.as_any().downcast_ref::<NetworkBroadcastExec>().is_some();
            if n_tasks == 1 && input_stage_info.task_count == 1 && !is_broadcast {
...
fn contains_shuffle_boundary(plan: &Arc<dyn ExecutionPlan>) -> bool {
    ...
    plan.children()
        .iter()
        .any(|child| contains_shuffle_boundary(child))
}
...
mod tests {
    ...
    */
    ...
    async fn test_left_join_distributed() {
        ...
        │           [Stage 7] => NetworkShuffleExec: output_partitions=4, input_tasks=3
        └──────────────────────────────────────────────────
          ┌───── Stage 6 ── Tasks: t0:[p0] 
          │ CoalescePartitionsExec
          │   [Stage 5] => NetworkCoalesceExec: output_partitions=8, input_tasks=2
          └──────────────────────────────────────────────────
            ┌───── Stage 5 ── Tasks: t0:[p0..p3] t1:[p0..p3] 
            ...
            └──────────────────────────────────────────────────
              ...
              └──────────────────────────────────────────────────
          ┌───── Stage 7 ── Tasks: t0:[p0..p3] t1:[p0..p3] t2:[p0..p3] 
          │ CoalesceBatchesExec: target_batch_size=8192
