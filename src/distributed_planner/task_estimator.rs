...
pub struct PlannedLeafNode {
    ...
    /// Optional per-task URLs for consistent-hashing or affinity-based routing.
    urls: Option<Vec<Url>>,
    /// Optional per-task specialized plans. When set, each task receives its own distinct
    /// serialized plan rather than sharing a single pre-serialized copy. This enables
    /// coordinator-side plan specialization (e.g. trimming non-owned file groups before
    /// the plan bytes are sent over the wire).
    plans: Option<Vec<Arc<dyn ExecutionPlan>>>,
}
...
impl PlannedLeafNode {
    ...
    /// Sets per-task specialized plans. `plans[i]` is sent to task `i` instead of the shared
    /// plan, enabling the coordinator to trim non-owned data (e.g. file groups) before
    /// serialization. The number of plans must equal `task_count`.
    pub fn with_plans(
        &mut self,
        plans: Vec<Arc<dyn ExecutionPlan>>,
        task_count: usize,
    ) -> Result<()> {
        if plans.len() != task_count {
            return Err(internal_datafusion_err!(
                "number of plans must be equal to number of tasks"
            ));
        }
    ...
    /// Returns the per-task specialized plans, if set.
    pub fn plans(&self) -> Option<Vec<Arc<dyn ExecutionPlan>>> {
        self.plans.clone()
    }
}
