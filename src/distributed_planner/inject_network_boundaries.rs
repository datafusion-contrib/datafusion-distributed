...
/// Returns true if this join cannot be distributed.
fn collect_left_hash_join_requires_single_task(
    ...
    // We expect the distributed query planner to have inserted
    // BroadCastExec if this join is safe to distribute. Not all
    // collect left hash joins are broadcastable.
    !processed_children
        .first()
...
async fn _inject_network_boundaries(
    ...
    if nb_ctx.d_cfg.children_isolator_unions && plan.is::<UnionExec>() {
        ...
        // Only distribute CollectLeft HashJoins when safe.
