...
use datafusion_distributed::{
    BoxCloneSyncChannel, ChannelResolver, DistributedExt,
    DistributedPhysicalOptimizerRule, create_flight_client,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let resolver = MultiWorkerInMemoryChannelResolver::new(5);
    let state = SessionStateBuilder::new()
        .with_default_features()
        .with_distributed_channel_resolver(resolver)
        .with_physical_optimizer_rule(Arc::new(DistributedPhysicalOptimizerRule))
        .with_distributed_network_shuffle_tasks(3)
        .with_distributed_network_coalesce_tasks(3)
        .with_config(SessionConfig::new().with_information_schema(true))
        .build();
    let session_ctx = SessionContext::from(state);
    ...
    > {
        ...
        if let Some(msg) = _request.metadata().get("create") {
            ...
            let state = SessionStateBuilder::new()
                ...
                .with_config(SessionConfig::new().with_information_schema(true))
                .build();
            let new_ctx = SessionContext::from(state);
...
/// Simple [ChannelResolver] implementation that simulates multiple workers
/// by returning multiple URLs but connecting to the same endpoint.
#[derive(Clone)]
struct MultiWorkerInMemoryChannelResolver {
    num_workers: usize,
}

impl MultiWorkerInMemoryChannelResolver {
...
#[async_trait]
impl ChannelResolver for MultiWorkerInMemoryChannelResolver {
    fn get_urls(&self) -> Result<Vec<url::Url>, DataFusionError> {
        // Return multiple URLs representing our 5 workers
        let urls: Result<Vec<url::Url>, _> = (0..self.num_workers)
            .map(|i| url::Url::parse(&format!("http://localhost:5005{}", i)))
            .collect();
        urls.map_err(|e| DataFusionError::External(Box::new(e)))
    }
    ...
    async fn get_flight_client_for_url(
        ...
        // For simplicity in the fuzz server, all workers connect to the same in-memory endpoint
        // In a real distributed setup, each URL would connect to a different worker
