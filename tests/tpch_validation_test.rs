...
mod tests {
    ...
    use datafusion_distributed::{
        assert_snapshot, explain_analyze, DistributedPhysicalOptimizerRule, DistributedSessionBuilderContext
    };
    ...
    async fn test_tpch_1() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(1).await?;
        ...
        └──────────────────────────────────────────────────
          ...
          └──────────────────────────────────────────────────
            ...
            └──────────────────────────────────────────────────
        ");
        assert_snapshot!(analyzed_plan, @r"
        ┌───── Stage 3    Task t0:[p0] 
        │ SortPreservingMergeExec: [l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], metrics=[output_rows=4, elapsed_compute=57.041µs]
        │   NetworkCoalesceExec, metrics=[] read_from=Stage 2, output_partitions=24, input_tasks=4
        └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t0:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=0, elapsed_compute=6ns, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=0, elapsed_compute=6ns]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=0, elapsed_compute=225.832µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=384]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=0, elapsed_compute=129ns]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t1:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=1, elapsed_compute=55.13µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=1, elapsed_compute=7.63µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=1, elapsed_compute=290.33µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=1776]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=6, elapsed_compute=77.502µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t2:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=1, elapsed_compute=21.089µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=1, elapsed_compute=2.505µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=1, elapsed_compute=287.334µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=1776]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=6, elapsed_compute=53.547µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t3:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=2, elapsed_compute=38.296µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=2, elapsed_compute=4.588µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=2, elapsed_compute=333.083µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=3168]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=12, elapsed_compute=93.501µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
            ┌───── Stage 1    Task t0:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=874.186667ms, repartition_time=111.96µs, send_time=34.957µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=157.91346ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=777840]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=222647, elapsed_compute=32.783335ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=222647, elapsed_compute=37.82129ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=222647, elapsed_compute=5.948708ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=225970, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2878038, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=225970, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=6, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=170.675µs, metadata_load_time=420.225548ms, page_index_eval_time=235.048µs, row_pushdown_eval_time=14ns, statistics_eval_time=279.839µs, time_elapsed_opening=252.851836ms, time_elapsed_processing=89.589793ms, time_elapsed_scanning_total=612.597626ms, time_elapsed_scanning_until_data=319.81504ms]
            └──────────────────────────────────────────────────
            ┌───── Stage 1    Task t1:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=1.67457025s, repartition_time=104.123µs, send_time=23.289µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=132.320832ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=778048]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=184909, elapsed_compute=27.536082ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=184909, elapsed_compute=31.606876ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=184909, elapsed_compute=4.928458ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=187652, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2391724, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=187652, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=5, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=134.175µs, metadata_load_time=1.250104383s, page_index_eval_time=185.425µs, row_pushdown_eval_time=14ns, statistics_eval_time=259.673µs, time_elapsed_opening=609.507334ms, time_elapsed_processing=74.961373ms, time_elapsed_scanning_total=1.060648663s, time_elapsed_scanning_until_data=812.711956ms]
            └──────────────────────────────────────────────────
            ┌───── Stage 1    Task t2:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=1.629081834s, repartition_time=111.209µs, send_time=26.707µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=133.745749ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=1035072]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=184300, elapsed_compute=27.577375ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=184300, elapsed_compute=31.772956ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=184300, elapsed_compute=5.420586ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=186950, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2383972, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=186950, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=5, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=149.339µs, metadata_load_time=971.483841ms, page_index_eval_time=220.841µs, row_pushdown_eval_time=14ns, statistics_eval_time=365.591µs, time_elapsed_opening=455.008834ms, time_elapsed_processing=77.600955ms, time_elapsed_scanning_total=1.169846538s, time_elapsed_scanning_until_data=917.656208ms]
            └──────────────────────────────────────────────────
        ");
        Ok(())
    }
    ...
    async fn test_tpch_2() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(2).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 8   Tasks: t0:[p0] 
    ...
    async fn test_tpch_3() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(3).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    async fn test_tpch_4() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(4).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    async fn test_tpch_5() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(5).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    async fn test_tpch_6() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(6).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 2   Tasks: t0:[p0] 
    ...
    async fn test_tpch_7() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(7).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    async fn test_tpch_8() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(8).await?;
        assert_snapshot!(plan, @r#"
        ┌───── Stage 9   Tasks: t0:[p0] 
    ...
    async fn test_tpch_9() -> Result<(), Box<dyn Error>> {
        let (plan, analyzed_plan) = test_tpch_query(9).await?;
        assert_snapshot!(plan, @r"
        ┌───── Stage 8   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(10).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(11).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 3   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(12).await?;
    //     assert_snapshot!(plan, @r#"
    //     ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(13).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(14).await?;
    //     assert_snapshot!(plan, @r#"
    //     ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(15).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(16).await?;
    //     assert_snapshot!(plan, @r#"
    //     ┌───── Stage 6   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(17).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(18).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 9   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(19).await?;
    //     assert_snapshot!(plan, @r#"
    //     ┌───── Stage 2   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(20).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 4   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(21).await?;
    //     assert_snapshot!(plan, @r"
    //     ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    //     let (plan, analyzed_plan) = test_tpch_query(22).await?;
    //     assert_snapshot!(plan, @r#"
    //     ┌───── Stage 5   Tasks: t0:[p0] 
    ...
    async fn run_tpch_query(ctx_d: SessionContext, query_id: u8) -> Result<(String, String), Box<dyn Error>> {
        ...
        let explain_analyze = explain_analyze(plan_d)?;
