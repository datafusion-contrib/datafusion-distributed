...
mod tests {
    ...
    async fn test_tpch_1() -> Result<(), Box<dyn Error>> {
        ...
        ┌───── Stage 3    Task t0:[p0] 
        │ SortPreservingMergeExec: [l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], metrics=[output_rows=4, elapsed_compute=62.291µs]
        ...
        └──────────────────────────────────────────────────
          ...
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=0, elapsed_compute=197.624µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=384]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=0, elapsed_compute=211ns]
          ...
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=1, elapsed_compute=35.296µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=1, elapsed_compute=3.047µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=1, elapsed_compute=293.083µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=1776]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=6, elapsed_compute=62.252µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t2:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=1, elapsed_compute=20.422µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=1, elapsed_compute=2.296µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=1, elapsed_compute=250.792µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=1776]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=6, elapsed_compute=68.376µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
          ┌───── Stage 2    Task t3:[p0,p1,p2,p3,p4,p5] 
          │ SortExec: expr=[l_returnflag@0 ASC NULLS LAST, l_linestatus@1 ASC NULLS LAST], preserve_partitioning=[true], metrics=[output_rows=2, elapsed_compute=38.17µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0]
          │   ProjectionExec: expr=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus, sum(lineitem.l_quantity)@2 as sum_qty, sum(lineitem.l_extendedprice)@3 as sum_base_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount)@4 as sum_disc_price, sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax)@5 as sum_charge, avg(lineitem.l_quantity)@6 as avg_qty, avg(lineitem.l_extendedprice)@7 as avg_price, avg(lineitem.l_discount)@8 as avg_disc, count(Int64(1))@9 as count_order], metrics=[output_rows=2, elapsed_compute=4.421µs]
          │     AggregateExec: mode=FinalPartitioned, gby=[l_returnflag@0 as l_returnflag, l_linestatus@1 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=2, elapsed_compute=258.044µs, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, peak_mem_used=3168]
          │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=12, elapsed_compute=100.127µs]
          │         NetworkShuffleExec, metrics=[] read_from=Stage 1, output_partitions=6, n_tasks=4, input_tasks=3
          └──────────────────────────────────────────────────
                ┌───── Stage 1    Task t0:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=1.0413095s, repartition_time=106.208µs, send_time=29.958µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=158.747417ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=777840]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=222647, elapsed_compute=32.960379ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=222647, elapsed_compute=37.423918ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=222647, elapsed_compute=5.75025ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=225970, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2878038, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=225970, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=6, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=157.048µs, metadata_load_time=557.249005ms, page_index_eval_time=207.634µs, row_pushdown_eval_time=14ns, statistics_eval_time=279.923µs, time_elapsed_opening=282.554584ms, time_elapsed_processing=89.537957ms, time_elapsed_scanning_total=750.111707ms, time_elapsed_scanning_until_data=457.152335ms]
            └──────────────────────────────────────────────────
            ┌───── Stage 1    Task t1:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=1.568928626s, repartition_time=92.794µs, send_time=28.416µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=129.837624ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=778048]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=184909, elapsed_compute=26.955667ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=184909, elapsed_compute=31.054705ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=184909, elapsed_compute=4.744041ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=187652, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2391724, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=187652, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=5, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=105.508µs, metadata_load_time=1.055738133s, page_index_eval_time=136.007µs, row_pushdown_eval_time=14ns, statistics_eval_time=206.591µs, time_elapsed_opening=454.162708ms, time_elapsed_processing=73.866538ms, time_elapsed_scanning_total=1.11071629s, time_elapsed_scanning_until_data=868.115334ms]
            └──────────────────────────────────────────────────
            ┌───── Stage 1    Task t2:[p0,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,p16,p17,p18,p19,p20,p21,p22,p23] 
            │ RepartitionExec: partitioning=Hash([l_returnflag@0, l_linestatus@1], 24), input_partitions=2, metrics=[fetch_time=1.569371083s, repartition_time=101.668µs, send_time=36.417µs]
            │   AggregateExec: mode=Partial, gby=[l_returnflag@5 as l_returnflag, l_linestatus@6 as l_linestatus], aggr=[sum(lineitem.l_quantity), sum(lineitem.l_extendedprice), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount), sum(lineitem.l_extendedprice * Int64(1) - lineitem.l_discount * Int64(1) + lineitem.l_tax), avg(lineitem.l_quantity), avg(lineitem.l_extendedprice), avg(lineitem.l_discount), count(Int64(1))], metrics=[output_rows=8, elapsed_compute=130.672083ms, spill_count=0, spilled_bytes=0.0 B, spilled_rows=0, skipped_aggregation_rows=0, peak_mem_used=1035072]
            │     ProjectionExec: expr=[l_extendedprice@1 * (Some(1),20,0 - l_discount@2) as __common_expr_1, l_quantity@0 as l_quantity, l_extendedprice@1 as l_extendedprice, l_discount@2 as l_discount, l_tax@3 as l_tax, l_returnflag@4 as l_returnflag, l_linestatus@5 as l_linestatus], metrics=[output_rows=184300, elapsed_compute=27.032332ms]
            │       CoalesceBatchesExec: target_batch_size=8192, metrics=[output_rows=184300, elapsed_compute=31.180961ms]
            │         FilterExec: l_shipdate@6 <= 1998-09-02, projection=[l_quantity@0, l_extendedprice@1, l_discount@2, l_tax@3, l_returnflag@4, l_linestatus@5], metrics=[output_rows=184300, elapsed_compute=4.593788ms]
            │           PartitionIsolatorExec, metrics=[]
            │             DataSourceExec: file_groups={6 groups: [[/testdata/tpch/data/lineitem/1.parquet:0..2263630, /testdata/tpch/data/lineitem/10.parquet:0..2247315, /testdata/tpch/data/lineitem/11.parquet:0..1497293], [/testdata/tpch/data/lineitem/11.parquet:1497293..2257665, /testdata/tpch/data/lineitem/12.parquet:0..2246562, /testdata/tpch/data/lineitem/13.parquet:0..2280549, /testdata/tpch/data/lineitem/14.parquet:0..720755], [/testdata/tpch/data/lineitem/14.parquet:720755..2262883, /testdata/tpch/data/lineitem/15.parquet:0..2235696, /testdata/tpch/data/lineitem/16.parquet:0..2230414], [/testdata/tpch/data/lineitem/16.parquet:2230414..2266608, /testdata/tpch/data/lineitem/2.parquet:0..2246598, /testdata/tpch/data/lineitem/3.parquet:0..2273582, /testdata/tpch/data/lineitem/4.parquet:0..1451864], [/testdata/tpch/data/lineitem/4.parquet:1451864..2243067, /testdata/tpch/data/lineitem/5.parquet:0..2258249, /testdata/tpch/data/lineitem/6.parquet:0..2240773, /testdata/tpch/data/lineitem/7.parquet:0..718013], ...]}, projection=[l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate], file_type=parquet, predicate=l_shipdate@6 <= 1998-09-02, pruning_predicate=l_shipdate_null_count@1 != row_count@2 AND l_shipdate_min@0 <= 1998-09-02, required_guarantees=[]
            │, metrics=[output_rows=186950, elapsed_compute=2ns, batches_splitted=0, bytes_scanned=2383972, file_open_errors=0, file_scan_errors=0, files_ranges_pruned_statistics=0, num_predicate_creation_errors=0, page_index_rows_matched=186950, page_index_rows_pruned=0, predicate_evaluation_errors=0, pushdown_rows_matched=0, pushdown_rows_pruned=0, row_groups_matched_bloom_filter=0, row_groups_matched_statistics=5, row_groups_pruned_bloom_filter=0, row_groups_pruned_statistics=0, bloom_filter_eval_time=118.714µs, metadata_load_time=881.420007ms, page_index_eval_time=141.714µs, row_pushdown_eval_time=14ns, statistics_eval_time=244.339µs, time_elapsed_opening=457.016124ms, time_elapsed_processing=73.40687ms, time_elapsed_scanning_total=1.108183211s, time_elapsed_scanning_until_data=864.164585ms]
            └──────────────────────────────────────────────────
        ");
        Ok(())
    }
