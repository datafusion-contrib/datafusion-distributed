    ...
    fn display_dynamic_filter_discovery(
        ...
        fn render(
            ...
            writeln!(
                ...
                "  ".repeat(depth),
                node.name(),
                dynamic_filter_annotations(node, normalizer)?,
            )
            .expect("writing to String cannot fail");
            for child in node.children() {
                render(child.as_ref(), depth + 1, normalizer, output)?;
            }
            Ok(())
        }
        ...
        for stage_id in plans.keys().sorted().rev() {
            ...
            render(plans[stage_id].as_ref(), 1, &mut normalizer, &mut output)?;
        }
        Ok(output)
    }
}
