...
def list_queries():
    ...
    try:
        ...
        queries = sorted(
            [f[:-4] for f in files if f.startswith("q") and f.endswith(".sql")]
        )
...
def explain_sql(sql_query):
    ...
    try:
        ...
        formatted_logical = (
            format_plan(logical_plan) if logical_plan else "Logical plan not available"
        )
        formatted_physical = (
            format_plan(physical_plan)
            if physical_plan
            else "Physical plan not available"
        )
        formatted_distributed = (
            format_plan(distributed_plan)
            if distributed_plan
            else "Distributed plan not available"
        )
        formatted_distributed_stages = (
            format_plan(distributed_stages)
            if distributed_stages
            else "Distributed stages not available"
        )
        print("\nExecution Plan:")
        print("=" * 100)
        print("Logical Plan:")
