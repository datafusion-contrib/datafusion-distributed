...
```shell
WORKERS=0 ./benchmarks-local/run.sh --threads 2 --dataset tpch_sf1
```
...
```shell
WORKERS=8 ./benchmarks-local/run.sh --threads 2 --dataset tpch_sf1 --file-scan-config-bytes-per-partition 16777216
```
