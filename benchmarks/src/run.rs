...
/// Run a local or distributed benchmark suite.
///
/// `--dataset` is `<suite>/<variant>` and is resolved to
/// `testdata/<suite>/<variant>`:
///
/// - `tpch/sf1` — TPC-H, generated by `./benchmarks/gen-tpch.sh`
/// - `tpcds/sf1` — TPC-DS, generated by `./benchmarks/gen-tpcds.sh`
/// - `clickbench/0-100` — ClickBench hits, generated by `./benchmarks/gen-clickbench.sh`
///
/// The suite root (`tpch`, `clickbench`, …) is not a valid data path; it holds
/// the shared `queries/` directory. Pass the generated variant.
...
pub struct RunOpt {
    ...
    /// Logical dataset name: `<suite>/<variant>`
    /// (e.g. `tpch/sf1`, `tpcds/sf1`, `clickbench/0-100`).
    /// Resolved to `testdata/<suite>/<variant>`.
    #[structopt(long)]
    dataset: String,
...
fn queries_for_dataset(dataset: &str) -> Result<Vec<(String, String)>, DataFusionError> {
    match dataset {
        ...
        "clickbench" => clickbench::get_queries()
            ...
            .collect(),
        _ => not_impl_err!(
            "Unknown benchmark dataset {dataset}. Supported suites: \
             clickbench, tpcds, tpch (pass --dataset <suite>/<variant>, \
             e.g. clickbench/0-100)"
        ),
    }
}

fn dataset_generation_hint(dataset: &str) -> &'static str {
    let suite = dataset.split_once('/').map_or(dataset, |(suite, _)| suite);
    match suite {
        "tpch" => {
            "Generate it with `./benchmarks/gen-tpch.sh` (writes testdata/tpch/sf<scale-factor>)."
        }
        "tpcds" => {
            "Generate it with `./benchmarks/gen-tpcds.sh` (writes testdata/tpcds/sf<scale-factor>)."
        }
        "clickbench" => {
            "Generate it with `./benchmarks/gen-clickbench.sh` (writes testdata/clickbench/<start>-<end>, default 0-100)."
        }
        _ => "See benchmarks/README.md for supported datasets.",
    }
}

/// Suite roots (`clickbench`, `tpch`, …) hold shared `queries/`; generated
/// parquet lives under a variant directory.
fn require_dataset_variant(dataset: &str) -> Result<(), DataFusionError> {
    if !dataset.contains('/') {
        return exec_err!(
            "--dataset {} is a suite name, not a generated variant. \
             Use a path like tpch/sf1, tpcds/sf1, or clickbench/0-100. {}",
            dataset,
            dataset_generation_hint(dataset)
        );
    }
    Ok(())
}
...
impl RunOpt {
    ...
    fn get_path(&self) -> Result<PathBuf> {
        require_dataset_variant(&self.dataset)?;
        ...
        if !data_path.exists() {
            return exec_err!(
                ...
                self.dataset,
                dataset_generation_hint(&self.dataset)
            );
        }
...
mod tests {
    ...
    #[test]
    fn suite_root_is_rejected_with_generation_hint() {
        let err = require_dataset_variant("clickbench").unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("suite name"), "{msg}");
        assert!(msg.contains("gen-clickbench.sh"), "{msg}");
        assert!(msg.contains("clickbench/0-100"), "{msg}");
    }

    #[test]
    fn variant_paths_are_accepted() {
        require_dataset_variant("clickbench/0-100").unwrap();
        require_dataset_variant("tpch/sf1").unwrap();
        require_dataset_variant("tpcds/sf1").unwrap();
    }

    #[test]
    fn unknown_suite_lists_supported_datasets() {
        let err = queries_for_dataset("imdb").unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("Unknown benchmark dataset imdb"), "{msg}");
        assert!(msg.contains("clickbench"), "{msg}");
        assert!(msg.contains("tpch"), "{msg}");
    }
}
